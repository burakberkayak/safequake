...
/**
 * AFAD ve Kandilli verilerini birleştiren Orhan Aydoğdu Deprem API'si için repository.
 * En güncel 100 deprem verisini tek bir istekte hızlıca çeker ve filtreleri uygular.
 */
    ...
    try {
      ...
      if (filters.minMagnitude !== undefined) {
        earthquakes = earthquakes.filter(
          (eq) => eq.magnitude >= filters.minMagnitude!,
        );
      }
    ...
    return raw
      .map((item: any) => {
        ...
        if (item.created_at) {
          ...
          const dateWithTz = dateStr.includes("+")
            ? dateStr
            : `${dateStr}+03:00`;
